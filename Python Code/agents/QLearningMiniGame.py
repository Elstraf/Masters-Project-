...
class Agent(base_agent.BaseAgent):
    ...
    def step(self, obs):
     ...
     #   targets = [unit for unit in obs.observation.raw_units
        ...
        #Checks to see if theyre not null
       # if marines and targets:
            #target = targets[0]
            #distances = self.get_distances(obs, marines, targets[0])
            #targetLocation = targets[np.argmax(distances)]
            #targetXY = [targets[0].x, targets[0].y]
           # return actions.RAW_FUNCTIONS.Attack_pt("now", marines,(targetXY))
